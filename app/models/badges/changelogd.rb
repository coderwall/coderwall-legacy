  ...
  class << self
    ...
    def create_assignments!(repos)
      repos.each do |repo_url|
        ...
        Fact.append!("#{repo_url}:changedlogd", "github:#{github_username}", 'Repo featured on Changelogd', Time.now, repo_url, %w(repo changedlog))
      end
    end
    ...
    def repos_in(url)
      ...
      doc.xpath('//post/link-description').map do |element|

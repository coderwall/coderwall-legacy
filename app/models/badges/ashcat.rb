class Ashcat < BadgeBase
  ...
  def reasons
    @reasons ||= begin
      fact = user.facts.detect { |fact| fact.tagged?('rails', 'contribution') }
      fact.name if fact
    end
  end
  ...
  def self.add_contributor(github_username, contributions=1)
    ...
    Fact.append!("#{repo_url}:#{github_username}", "github:#{github_username}", name, Time.now, repo_url, ['rails', 'contribution'])
  end
end
class Notifier < ActionMailer::Base
  ...
  def new_badge(username)
    ...
    unless @badge.nil?
      ...
      fail NothingToSendException.new
    end
  end
  ...
  def activity_message_for_user(user)
    fail "Failed notifying user because there was no new activity for #{user.username}" unless user.activity_since_last_visit?
  ...
  def new_badge_message_for_user(_user, badge)

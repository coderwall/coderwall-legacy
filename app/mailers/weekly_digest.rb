class WeeklyDigest < ActionMailer::Base
  ...
  def weekly_digest(username)
    ...
    benchmark 'digest:stats' do
      @stats = @user.activity_stats(since, true).sort_by { |_stat, count| -(count || 0) }
    end
  ...
  def weekly_digest_subject_for(_user, stats, _most)
  ...
  def star_stats(stars, since = 1.week.ago)
    stars.map { |star| star.activity_stats(since, true) }.each_with_index.map { |stat, index| stat.merge(user: stars[index]) }
  end
  ...
  def get_team_and_job_for(user)
    if user.team.try(:hiring?)
      ...
      teams.each do |team|
        best_job = team.best_positions_for(user).find { |job| (job.team_document_id == user.team_document_id) or !already_sent?(job, user) }
